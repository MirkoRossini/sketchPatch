...
def get_topics_for_forum(forum, is_moderator, off, count):
  ...
  for t in topics:
    posts = Post.gql("WHERE forum = :1 AND topic = :2 AND is_deleted = False ORDER BY created_on", forum, t).fetch(1000)
    if len(posts) == 0:
      t.updated_time = t.updated_on
    else:
      t.updated_time = posts[-1].created_on
